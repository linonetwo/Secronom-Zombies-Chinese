...
const sourceModDirSuffix = '';
const cnDirSuffix = '_CN';
const getCNDirName = (filePath, sourceModDir) =>
  path.join(`${sourceModDir}${cnDirSuffix}`, filePath.replace(sourceModDir, ''));

const sourceModDirs = [`Secronom${sourceModDirSuffix}`];
...
function writeToCNMod(foldersWithContent, sourceModDir) {
  for (const inspectDataWithContent of foldersWithContent) {
    const newFilePath = getCNDirName(inspectDataWithContent.filePath, sourceModDir);
